...
export function Graph({ graphType, duration, onMove, isLoading, data }: Props): JSX.Element | null {
  ...
  if (isSeriesGraph && isSeriesDuration(duration) && isSeriesData(data)) {
    return (
      <SeriesGraph className="relative my-5 h-80" data={data} duration={duration} onMove={onMove} />
    );
  }
