...
export const CandleStickGraph: React.FC<Props> = React.memo(function Graph({
  ...
  const renderTooltip = useCallback(
    (param: MouseEventParams) => {
      ...
      refTooltip.current.innerHTML = `<div><div class="w-3 h-3 rounded-full inline-block mr-1 ${
        isGreen ? 'bg-green' : 'bg-orange'
      }"></div>${DateTime.fromISO(date.toISO()).toFormat('dd/MM/yyyy hh:mm')}</div>
        <div>Open: ${formatNumber(String(openPrice))}</div>Close: ${formatNumber(
        String(closePrice)
      )}</div>`;
  ...
  useEffect(() => {
    ...
    if (containerNode) {
      ...
      //@ts-ignore
      refTooltip.current.style = Tooltip.toString();
    }
  ...
  }, [refTooltip, updateContainerBounds, ref]);
