...
export const SelectToken: React.FC<SelectTokenProps> = observer(
  ({ getLinkUrl, setTokenToStore, closeSelect, openSelectCustomToken, openCustomConfirmation }) => {
    ...
    const tokens = Object.values(appStore.tokens ?? {});
    ...
    const tokenListWithBalances = useMemo(() => {
      ...
      if (appStore.tokens && isTokensRecieved) {
        Object.entries(appStore.tokens).forEach(([tokenAddress, token]) => {
    ...
    }, [appStore.tokens, swapStore?.tokenBalances]);

    const filteredTokens = useMemo(() => {
      //without account
      if (!web3Store.accountAddress || !tokenListWithBalances.length) {
        return (appStore.tokens && tokens.filter(filterToken)) ?? [];
      }
      //account connected
      ...
      return [];
    }, [tokenListWithBalances, appStore.tokens, tokens, filterToken, web3Store?.accountAddress]);
    ...
    return (
      ...
      >
        ...
        />
        {isSearchByAddress ? (
          <CustomTokenItem address={filterValue} openCustomConfirmation={openCustomConfirmation} />
        ) : (
          <TokenList
